export const questions = [
   {
      questionText: '놀란 가슴을 진정 시키고 밖을 나와 보니 당신 눈 앞에는 환하고 아름다운 숲이 펼쳐져 있다 .이 때 당신의 반응은?',
      url: 'img/1q.png',
      answerOptions: [
         {
            answerText: '우와... 여기 뭐야? 겁나 이쁘네?',
            resultText: '개발자를 하면 늘 새로운 환경일거야',
            option: 'front',
         },
         {
            answerText: '뭐지... 여기 어디야? 집갈래..',
            resultText: '개발언어 하나만 배우면 대부분 비슷비슷해',
            option: 'back',
         },
      ],
   },
   {
      questionText: '떨어진 내집 아래에 깔려 있는 마녀의 구두를 보고 드는 생각은?',
      url: 'img/2q.png',
      answerOptions: [
         {
            answerText: '꺅!! 어떻게 사람이 깔렸어요! 도와주세요!',
            resultText: '협업할때는 맡은 역할을 잘 수행해내는 팀원의 역할이 어울려',
            option: 'front',
         },
         {
            answerText: '사람이 깔렸지만.. 구두가 예쁘네? 내가 신어야지',
            resultText: '협업할때는 일을 정해주는 팀장역할이 어울려',
            option: 'back',
         },
      ],
   },
   {
      questionText: '북쪽 마녀가 집으로 돌아가기 위해서는 험난한 길을 지나 모두가 두려워 하는 마법사를 만나야 한다고 한다 당신의 반응은?',
      url: 'img/3q.png',
      answerOptions: [
         {
            answerText: '마법사? 우와 나 마법볼수 있는거야? 재밌겠네?',
            resultText: '일단 여러가지를 배우고 길을 정하는게 좋을듯',
            option: 'front',
         },
         {
            answerText: '음.. 나는 마법도 안좋아하고 힘든거 싫은데..',
            resultText: '일단 목표를 잡고 선택과 집중을 하는게 좋을듯',
            option: 'back',
         },
      ],
   },
   {
      questionText: '마법사에게로 향하는 도중 누군가가 말을 걸어 돌아보니 움직이는 허수아비가 자신도 뇌를 갖고 싶다고 한다. 당신의 반응은?',
      url: 'img/4q.png',
      answerOptions: [
         {
            answerText: '그래? 나랑 같이 다니면서 마법사 만나면 만들어달라 하자',
            resultText: '선생님쪽도 나쁘지 않아',
            option: 'front',
         },
         {
            answerText: '지푸라기가 뇌 가져서 뭐하게? 계획이 있으니까 일단 따라와봐',
            resultText: '창업쪽도 나쁘지 않아',
            option: 'back',
         },
      ],
   },
   {
      questionText: '허수아비와 함께 길을 걷다 녹슬어 버린 양철 나무꾼을 만나게 된다. 이야기를 나누어 보니 일년 간 꼼짝 못했다고 하는데 이때 당신의 반응은?',
      url: 'img/5q.png',
      answerOptions: [
         {
            answerText: '어이구야.. 힘들었겠다.. 나는 절대 가만히 못있어',
            resultText: '여러 방식을 다 해봐',
            option: 'front',
         },
         {
            answerText: '등 긁어줄까?',
            resultText: '한곳만 죽어라 파자',
            option: 'back',
         },
      ],
   },
   {
      questionText: '무시무시 해보이는 모습과는 달리 겁이 많은 사자를 만났다 이 때 사자에게 건내주고 싶은 말은?',
      url: 'img/6q.png',
      answerOptions: [
         {
            answerText: '너는 보기와 달리 겁이 많구나!! 그럴 수 있지',
            resultText: '일단 해',
            option: 'front',
         },
         {
            answerText: '너 왜이리 겁이많니? 사자 맞아?',
            resultText: '하기전에 한번만 고민하자',
            option: 'back',
         },
      ],
   },
   {
      questionText: '마녀를 만났는데 늑대에게 우릴 공격하라고 명령하여 늑대가 우릴 에워싼 상태에서 나의 반응은?',
      url: 'img/7q.png',
      answerOptions: [
         {
            answerText: '겁에 질려 머리가 새하얘진다.',
            resultText: '아니다 싶으면 그게 맞아',
            option: 'front',
         },
         {
            answerText: '유서를 쓴다.',
            resultText: '맞다 싶으면 그게 아니야',
            option: 'back',
         },
      ],
   },
   {
      questionText: '끝없이 이어진 죽음의 양비귀 꽃밭에 들어간 당신, 어떤쪽에 속하는가? ',
      url: 'img/8q.png',
      answerOptions: [
         {
            answerText: '확률적으로 잠이 쏟아지기전 나갈 수 없다고 판단해 누가 구해줄수 있도록 의연히 기다린다. ',
            resultText: '주변의 사람들이 중요해',
            option: 'front',
         },
         {
            answerText: '자신의 선택을 후회하며 자포자기 한다.',
            resultText: '주변의 사람들을 잘둬',
            option: 'back',
         },
      ],
   },
   {
      questionText: '주문을 3번 외우면 날개 달린 원숭이를 부릴수 있는 모자가 생겼다. 당신은 무엇부터 할 것인가?',
      url: 'img/9q.png',
      answerOptions: [
         {
            answerText: '당장 가장 먼 곳으로 해외 여행을 떠나볼까?',
            resultText: '심심할땐 해외도 가고',
            option: 'front',
         },
         {
            answerText: '가장 효율적으로 부려먹을 수 있는 방법을 찾아 볼까?',
            resultText: '심심할땐 다른 부수익도 생각해보고',
            option: 'back',
         },
      ],
   },
   {
      questionText: '에메랄드시에 도착한 당신, 도시가 온통 초록색이다. 이때 먼저 든 생각은? ',
      url: 'img/10q.png',
      answerOptions: [
         {
            answerText: '우와.. 세상이 온통 초록색이다~ (받아들인다)',
            resultText: '이게 왜 안돼? 금지',
            option: 'front',
         },
         {
            answerText: '뭐지.. 내눈이 잘못되었나.. (받아들이지 못한다)',
            resultText: '이게 왜 돼? 금지',
            option: 'back',
         },
      ],
   },
   {
      questionText: '당신이 마주한 오즈의 모습은?',
      url: 'img/11q.png',
      answerOptions: [
         {
            answerText: '몸은 없고 얼굴만 있는 오즈',
            resultText: '상대 눈치는 그만보고',
            option: 'front',
         },
         {
            answerText: '뾰족하고 날카로운 모자와 신발, 어두운 색의 코트를 걸친 늙은 마녀',
            resultText: '상대 눈치좀 보고',
            option: 'back',
         },
      ],
   },
   {
      questionText: '당신은 드디어 집으로 돌아갈 수 있게 되었다. 돌아간다면 무엇을 할 것인가?',
      url: 'img/12q.png',
      answerOptions: [
         {
            answerText: '드디어 집으로 돌아간다. 빨리 보내줘!',
            resultText: '어쨋든 넌 뭐든 해낼거야',
            option: 'front',
         },
         {
            answerText: '오즈의 나라에서 신분 세탁도하고 새로운 삶을 살겠다.',
            resultText: '어쨋든 넌 부자가 될거야',
            option: 'back',
         },
      ],
   },
];
